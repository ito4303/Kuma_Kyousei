...
# 石川県の行政区域データの読み込み
kaga <- sf::st_read(ishikawa_pref_file, quiet = TRUE) |>
  # 加賀地方（かほく市以南）の市町を抽出します。
  dplyr::filter(N03_004 %in% c("金沢市", "小松市", "加賀市", "かほく市",
                               "白山市", "能美市", "野々市市", "川北町",
                               "津幡町", "内灘町")) |>
  sf::st_union()

# 土地利用データのうち加賀地方に重なる部分を抽出
int <- sf::st_intersects(landuse, kaga, sparse = FALSE)
landuse_kaga <- landuse[int, ]
...
ggplot(land_prop, aes(x = `河川地及び湖沼`, y = prop)) +
  geom_point(color = "red", size = 2.5) +
  labs(title = "加賀地方における、河川地及び湖沼面積とクマ出没頻度との関係",
...
# ブナ豊凶との関係
tidyr::expand_grid(
  mesh_code = unique(landuse_kaga$mesh_code),
  year = 2019:2024
) |>
  ...
  labs(title = "加賀地方における、各年の3次メッシュごとの出没件数",
       x = "年",
       y = "出没件数") +
...
pred |>
  ...
  labs(title = "2025年の加賀地方のクマ出没確率の予測値") +
  theme_minimal(base_family = "Noto Sans JP")
