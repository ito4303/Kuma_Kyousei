...
# 小松市の土地利用データから、3次メッシュコードと地理情報だけの
# データを作成
...
# 小松市の、3次メッシュコードごとの、6年間の出没ありの年の割合を
# 地図として可視化
geometry |>
  ...
  labs(title = "2019〜2024年の小松市のクマ出没ありの年の割合") +
...
# モデルの作成とあてはめ
# モデル1: 森林面積(F)とその2乗、河川・湖沼面積(W)、ブナの豊凶(B)を説明変数とするモデル
# モデル2: 森林面積(F)とその2乗、ブナの豊凶を説明変数とするモデル
# ともに、これら説明変数のほかに、空間および時間の変量効果をモデルに組み込んでいます。
...
# AICの値はモデル2の方が多少小さく、またモデル2の方がより単純なモデルですので、
...
pred_buna_poor |>
  ...
  labs(title = "2025年の小松市のクマ出没確率の予測値") +
  theme_minimal(base_family = "Noto Sans JP")
